...
exports.createCourse = async (req, res) => {
  try {
    ...
    try {
      ...
      return res.status(400).json({
        success: false,
        message: "Invalid JSON data in tag or instructions",
      });
    }
    ...
    console.log("Before instructor not found");
  ...
  try {
    ...
    const instructorCourses = await Course.find({
      ...
      .populate({
        ...
        populate: {
          path: "subSection",
          select: "timeDuration", // Only select timeDuration for duration calculation
        },
      })
      .exec();
      ...
      const coursesWithDuration = instructorCourses.map((course) => {
        ...
        // Calculate total duration by summing up each subSection time
      ...
      // Return course data with added courseDuration field
      return {
        ...course.toObject(),
